...
// Ensure wishlistedIds is initialized as an array ref
const wishlistedIds = ref<number[]>([]);

function getHeartClass(id: number) {
  // Safe null checks
  if (!wishlistedIds.value || !Array.isArray(wishlistedIds.value)) {
    return 'text-[#536B59]';
  }
  return wishlistedIds.value.includes(id) 
...
function getImageSrc(images: any) {
  ...
  return images.img1 || Object.values(images)[0] || '';
}

function getCategoryClass(cat: string) {
  const isSelected = activeCategory.value === cat;
  return isSelected
...
>
      ...
      >
        ...
        >
          ...
          <div class="h-72 overflow-hidden bg-[#F4F8F1] relative">
            ...
            <!-- Wishlist Button -->
           <button
  v-on:click.stop.prevent="toggleWishlist(item.id)"
  ...
  v-bind:aria-label="'Add ' + item.name + ' to wishlist'"
>
  <Heart
    class="h-4 w-4 transition-colors duration-200"
    v-bind:class="getHeartClass(item.id)"
...
<!-- Clickable Image -->
<RouterLink
  v-bind:to="'/products/' + item.id"
  class="h-full w-full justify-center flex items-center"
>
  <img
    v-bind:src="getImageSrc(item.images)"
